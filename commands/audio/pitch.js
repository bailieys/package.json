...
{
    name: 'pitch',
    aliases: ['chipmunk', 'slowmo', 'nightcore'],
    category: 'audio',
    description: 'Change audio pitch (chipmunk/slow/nightcore)',
    usage: '.pitch <up|down|chipmunk|nightcore> (reply to audio)',
    execute: async (sock, message, args, context) => {
      ...
      if (!media) {
        return sock.sendMessage(chatId, {
          text: buildHint('Reply to audio or video', 'Usage: .pitch chipmunk — options: up, down, chipmunk, nightcore')
        }, { quoted: fake });
      }

      const mode = (args[0] || 'up').toLowerCase();
      const filterMap = {
        up:        'asetrate=44100*1.25,aresample=44100',
        down:      'asetrate=44100*0.75,aresample=44100',
        chipmunk:  'asetrate=44100*1.7,aresample=44100',
        nightcore: 'asetrate=44100*1.35,aresample=44100',
        slowmo:    'asetrate=44100*0.5,aresample=44100',
      };
      const filter = filterMap[mode] || filterMap.up;
      ...
      try {
        ...
        const pitched = await applyAudioEffect(inputBuf, ext, filter);
