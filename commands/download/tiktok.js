...
{
    name: 'tiktok',
    aliases: ['tt', 'tk', 'tikdl', 'tiktokdl', 'tiktokvideo'],
    category: 'download',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url) {
        return sock.sendMessage(chatId, { text: buildHint('Provide a TikTok link', '.tiktok <url>') }, { quoted: fake });
      }

      const tiktokPattern = /tiktok\.com\/|vm\.tiktok\.com\/|vt\.tiktok\.com\//;
      if (!tiktokPattern.test(url)) {
        return sock.sendMessage(chatId, { text: buildHint('Invalid TikTok link') }, { quoted: fake });
      }
      ...
      try {
        await sock.sendMessage(chatId, {
          video: result.buffer || { url: result.video },
          mimetype: 'video/mp4',
        ...
        try {
          ...
          await sock.sendMessage(chatId, { video: buf, mimetype: 'video/mp4', caption: `*${botName}*`, ...replyOpts() }, { quoted: fake });
          await sock.sendMessage(chatId, { react: { text: '✅', key: message.key } });
        } catch (e2) {
          await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
          await sock.sendMessage(chatId, { text: buildHint('Failed to send video.') }, { quoted: fake });
        }
      }
    }
  }
