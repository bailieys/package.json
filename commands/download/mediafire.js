...
{
    name: 'mediafire',
    aliases: ['mf', 'mfire', 'mfiredl', 'mediafiredl'],
    category: 'download',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url) {
        return sock.sendMessage(chatId, {
          text: buildHint('Provide a MediaFire link', '.mediafire <url>')
        }, { quoted: fake });
      }

      if (!url.includes('mediafire.com')) {
        return sock.sendMessage(chatId, {
          text: buildHint('Not a valid MediaFire link.')
        }, { quoted: fake });
      }

      await sock.sendMessage(chatId, { react: { text: '⏳', key: message.key } });
      try {
        const keith = await keithFirstMedia(KEITH_MEDIAFIRE_ENDPOINTS, url, 120000);
        if (keith?.buffer) {
          const nameFromUrl = decodeURIComponent((keith.url.split('/').pop() || '').split('?')[0]) || `file_${Date.now()}`;
          await sock.sendMessage(chatId, {
            document: keith.buffer,
