...
module.exports = [
{
    ...
    aliases: ['music'],
    category: 'download',
    description: 'Download a song as MP3 document',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!query) {
        return sock.sendMessage(chatId, {
          text: buildHint('Provide a song name', '.play <song name>')
        }, { quoted: fake });
      }
      ...
      try {
        const audio = await fetchYtAudioByQuery(query);
        if (!audio || !audio.download) throw new Error('Audio download failed');
        ...
        await sock.sendMessage(chatId, {
          document: audio.buffer || { url: audio.download },
          mimetype: 'audio/mpeg',
        ...
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        await sock.sendMessage(chatId, { text: buildHint(error.message) }, { quoted: fake });
      }
    }
  }
];
