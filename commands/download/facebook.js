...
{
    name: 'facebook',
    aliases: ['fb', 'fbdl', 'fbvideo', 'facebookdl'],
    category: 'download',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url || (!url.includes('facebook.com') && !url.includes('fb.watch'))) {
        return sock.sendMessage(chatId, { text: buildHint('Provide a Facebook link', '.facebook <url>') }, { quoted: fake });
      }
      ...
      try {
        await sock.sendMessage(chatId, { video: result.buffer || { url: result.video }, mimetype: 'video/mp4', caption: `*${botName}*\n${result.title || ''}`, ...replyOpts() }, { quoted: fake });
        await sock.sendMessage(chatId, { react: { text: '✅', key: message.key } });
      } catch (e) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
