...
{
    name: 'pinterest',
    aliases: ['pindl', 'pin', 'pinterestdl'],
    category: 'download',
    description: 'Download Pinterest images or videos',
    usage: '.pinterest <url>',
    execute: async (sock, message, args, context) => {
      ...
      if (!url || !/(?:pinterest\.|pin\.it\/)/i.test(url)) {
        return sock.sendMessage(chatId, { text: buildHint('Provide a Pinterest link', '.pinterest <url>') }, { quoted: fake });
      }
      await sock.sendMessage(chatId, { react: { text: '⏳', key: message.key } });
      const result = await keithFirstMedia(KEITH_PINTEREST_ENDPOINTS, url, 120000);
      if (!result?.buffer) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        return sock.sendMessage(chatId, { text: buildHint('Download failed! Try again later.') }, { quoted: fake });
      }
      try {
        const rawType = String(result.raw?.type || result.raw?.format || result.raw?.mime || '').toLowerCase();
        const isVideo = rawType.includes('video') || /\.(?:mp4|webm|mov)(?:\?|$)/i.test(result.url);
        if (isVideo) {
          await sock.sendMessage(chatId, { video: result.buffer, mimetype: 'video/mp4', caption: `*${botName}*`, ...replyOpts() }, { quoted: fake });
        } else {
          await sock.sendMessage(chatId, { image: result.buffer, caption: `*${botName}*`, ...replyOpts() }, { quoted: fake });
        }
        await sock.sendMessage(chatId, { react: { text: '✅', key: message.key } });
      } catch (error) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        await sock.sendMessage(chatId, { text: buildHint('Failed to send media.') }, { quoted: fake });
      }
    }
  }
];
