...
{
    name: 'spotify',
    aliases: ['spdl', 'spotifydl', 'spoti', 'spt'],
    category: 'download',
    description: 'Download Spotify audio',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url || !/open\.spotify\.com\//i.test(url)) {
        return sock.sendMessage(chatId, { text: buildHint('Provide a Spotify track link', '.spotify <url>') }, { quoted: fake });
      }
      await sock.sendMessage(chatId, { react: { text: '⏳', key: message.key } });
      const result = await keithFirstMedia(KEITH_SPOTIFY_ENDPOINTS, url, 120000);
      if (!result?.buffer) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        return sock.sendMessage(chatId, { text: buildHint('Download failed! Try again later.') }, { quoted: fake });
      }
      try {
        const title = (result.title || 'spotify-audio').replace(/[^\w\s-]/g, '').replace(/\s+/g, ' ').trim().substring(0, 80) || 'spotify-audio';
        await sock.sendMessage(chatId, { audio: result.buffer, mimetype: 'audio/mpeg', fileName: `${title}.mp3`, ...replyOpts() }, { quoted: fake });
        await sock.sendMessage(chatId, { react: { text: '✅', key: message.key } });
      } catch {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        await sock.sendMessage(chatId, { text: buildHint('Failed to send audio.') }, { quoted: fake });
      }
    }
  }
];
