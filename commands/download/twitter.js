...
{
    name: 'twitter',
    aliases: ['tw', 'xvideo', 'xdl', 'twtdl', 'tweetdl', 'twitterdl'],
    category: 'download',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url || (!url.includes('twitter.com') && !url.includes('x.com') && !url.includes('t.co'))) {
        return sock.sendMessage(chatId, {
          text: buildHint('Provide a Twitter/X link', '.twitter <url>')
        }, { quoted: fake });
      }
      ...
      if (!result?.video) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
        return sock.sendMessage(chatId, {
          text: buildHint('Download failed! Try again later.')
        }, { quoted: fake });
      }
