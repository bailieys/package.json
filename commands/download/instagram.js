...
{
    name: 'instagram',
    aliases: ['ig', 'insta', 'igdl', 'igvideo', 'instagramdl'],
    category: 'download',
    ...
    execute: async (sock, message, args, context) => {
      ...
      if (!url || !url.includes('instagram.com')) {
        return sock.sendMessage(chatId, { text: buildHint('Provide an Instagram link', '.instagram <url>') }, { quoted: fake });
      }
      ...
      try {
        if (result.video) {
          await sock.sendMessage(chatId, { video: result.buffer || { url: result.video }, mimetype: 'video/mp4', caption: `*${botName}*`, ...replyOpts() }, { quoted: fake });
        } else if (result.image) {
          await sock.sendMessage(chatId, { image: result.buffer || { url: result.image }, caption: `*${botName}*`, ...replyOpts() }, { quoted: fake });
        }
        await sock.sendMessage(chatId, { react: { text: '✅', key: message.key } });
      } catch (e) {
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
